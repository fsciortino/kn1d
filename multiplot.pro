...
;
Pro Multiplot,X1,Y1,X2,Y2,X3,Y3,X4,Y4,X5,Y5,X6,Y6,$
...
common multiplotR,RX1,RY1,RX2,RY2,RX3,RY3,RX4,RY4,RX5,RY5,RX6,RY6
;________________________________________________________________________________
;
; Input Variables:
...
for i=0,np-1 do begin
   ...
   if keyword_set(varlabel) then xyouts,xleg,yleg,/norm,align=0.0,varlabel(i),charsize=1.0*_mag,color=color(i)
   ix=0 & iy=0
   status=execute('ix=type_of(OX'+sval(i+1)+')')
   status=execute('iy=type_of(OY'+sval(i+1)+')')
   if ix ne 0 and iy ne 0 then begin
      status=execute('oplot,comb(OX'+sval(i+1)+',npts=npts),comb(OY'+sval(i+1)+',npts=npts),psym=psymbol(opsym(i)),color=ocolor(i)')
      if keyword_set(ovarlabel) then xyouts,xleg,yleg-.05,/norm,align=0.0,ovarlabel(i),charsize=1.0*_mag,color=ocolor(i)
   endif
   ix=0 & iy=0
   status=execute('ix=n_elements(RX'+sval(i+1)+')')
   status=execute('iy=n_elements(RY'+sval(i+1)+')')
   if ix gt 1 and iy gt 1 then begin
      ...
      if keyword_set(Rvarlabel) then xyouts,oxleg,yleg,/norm,align=1.0,rvarlabel(i),charsize=1.0*_mag,color=rcolor(i)
   endif
   ii=where(xmark lt 1.0e32,count)
   if count gt 0 then begin
      for k=0,count-1 do begin
         yr=!y.crange
         if ylog(i) then yr=10^yr
         oplot,[xmark(k),xmark(k)],yr,linestyle=5
      endfor
   endif
endfor
!p.charsize=0.0
